...
| FR-18 | **Creating a profile is an explicit, separate action from signing in** ("Create Account" vs. "Sign In" — not a single combined form). **Revised 2026-08-06** after stakeholder feedback: an earlier version of this requirement combined create+sign-in into one action, which meant a typo'd/unrecognized name silently created a throwaway account instead of erroring — confirmed as unwanted behavior. Creating a profile: if the entered display name does not already exist, the system creates a new profile with the given name and PIN. If the name is already taken, creation fails with a clear error (e.g., "That name is already taken — sign in instead or choose another name") and no existing profile is modified. |
| FR-19 | **Signing in**: requires an existing display name + matching PIN. If the name doesn't exist, or exists but the PIN doesn't match, sign-in fails with a single clear error that doesn't distinguish the two cases (e.g., "That username and PIN aren't recognized") — deliberately not revealing whether the name exists, so sign-in can never be used to silently create an account, and doesn't leak which names are taken. No profile is created or modified on failure. |
| FR-20 | *(superseded by FR-19's unified failure case above — signing in no longer distinguishes "wrong PIN" from "name not found"; both are rejected identically.)* |
